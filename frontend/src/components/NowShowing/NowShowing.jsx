...
export default function NowShowing({ showAll = false, loadedMovies = [] }) {
  ...
  const fetchMovies = async () => {
    ...
    try {
      setLoading(true);
      const response = await fetch('http://localhost:4000/api/movies/nowshowing');
    ...
    <div className=" flex flex-col justify-center gap-8 my-4 px-8  mx-auto">
      ...
      {!showAll && (
        <div className="w-full flex justify-center items-center">
          <Link to="/movies/now-showing" className="bg-tertiary p-4 rounded-md font-bold text-white hover:cursor-pointer hover:bg-tertiary/80 transition-all duration-300">
