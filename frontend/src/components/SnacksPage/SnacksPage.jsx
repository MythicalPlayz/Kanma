        ...
        {food.map((category, index) => (
          <div key={index} className="flex flex-col gap-4">
            <div className="flex justify-start items-center gap-2 w-full">
              <FontAwesomeIcon icon={category.faIcon} size='xl' />
              <h3 className='font-bold text-3xl'>{lang === 'en' ? category.categoryEN : category.categoryAR}</h3>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-4">
              {category.items.map((item, itemIndex) => (
                <div key={itemIndex} className="flex flex-col gap-2 justify-start items-start p-4 border-tertiary border-4 bg-secondary rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300">
                  <img src={item.imageURL} alt={lang === 'en' ? item.nameEN : item.nameAR} className="w-full h-50 object-cover rounded-lg" />
